...
async function handleInit(opts: OnboardOpts, meta: CliMetadata) {
  ...
  if (existingCredentials && !opts.switch) {
    ...
    throw new Error(
      `Refusing to overwrite existing Sponge credentials for ${currentAgent}. ` +
...
const EVM_CHAIN_VALUES = ["ethereum", "base", "arbitrum-one", "polygon", "sepolia", "base-sepolia", "arbitrum-sepolia", "polygon-amoy"] as const;
...
async function executeMppSessionStreamCommand(
  opts: WalletSessionOpts,
  input: {
    session_id: unknown;
    url: unknown;
  ...
  const response = await wallet.streamMppSessionRequest({
    ...
    method: input.method as "GET" | "POST" | "PUT" | "DELETE" | "PATCH" | undefined,
...
async function pipeResponseBodyToStdout(response: Response): Promise<void> {
