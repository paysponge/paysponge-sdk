...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
    ...
    input_schema: {
      ...
      properties: {
        ...
        symbol: {
          type: "string",
          description:
            "CCXT symbol (e.g., 'BTC/USDC:USDC' for default perps, 'PURR/USDC' for spot, 'vntl:ANTHROPIC' for HIP-3/builder perps)",
        },
        side: {
          type: "string",
          enum: ["buy", "sell"],
        ...
        query: {
          type: "string",
          description:
            "Filter markets by symbol/base/quote/dex substring. Use this to discover natural-language assets and HIP-3/builder perps before answering that a market is unavailable.",
        },
        market_type: {
          type: "string",
          enum: ["spot", "swap"],
  ...
  },
  {
    ...
    description:
      "Store credit card details as a normal encrypted payment method for this agent. " +
  ...
  },
  {
    ...
    description:
      "Start Sponge Card onboarding for the authenticated user, returning a KYC verification link first when internal KYC is missing, and record Sponge consent acknowledgements. " +
